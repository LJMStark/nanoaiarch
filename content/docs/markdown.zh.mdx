...
```mdx
<Callout title="标题">你好，世界</Callout>
```
...
每个标题会自动应用锚点，它会清理空格等无效字符。（例如，`你好，世界` 变为 `hello-world`）
