...
仓库为 public，**测试账号绝不写入 git 跟踪文件**。账密统一放在本地 `.env.local`（已被 `.gitignore` 覆盖）：

- `TEST_USER_EMAIL` — 工作台烟雾测试用户邮箱
- `TEST_USER_PASSWORD` — 对应密码

部署后冒烟流程：登录 → 进入工作台 → 选/建项目 → 生成一张图 → 验证下载/分享/编辑按钮 → 检查历史面板「清除」二次确认 → 桌面 hover overlay + 移动操作行各点一次。
